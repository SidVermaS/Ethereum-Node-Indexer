...
func GetStates(ctx *fiber.Ctx) error {
	...
	states, err := modules.GetFilteredStates(queries, offset, limit)

	if err != nil {
		ctx.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"message": "Something went wrong..."})
		return err
	}
	// Returns an array of the states in JSON format
	ctx.Status(fiber.StatusOK).JSON(fiber.Map{"states": states})
	return nil
}
